# Lassare Documentation

Official documentation for [Lassare](https://lassare.com) - human-in-the-loop workflows for AI agents.

## Coming Soon

Documentation is being prepared. Check back soon!

## Structure

```
docs/
├── getting-started/     # Quickstart guides
├── guides/              # How-to guides
├── api/                 # API reference
├── sdks/                # SDK documentation
└── concepts/            # Core concepts
```

## Community & Support

- **Questions & Discussions**: [GitHub Discussions](https://github.com/lassare-hq/docs/discussions)
- **Bug Reports**: Open an issue on the relevant repo
- **Paid Support**: Available on Team, Growth, and Enterprise plans

## Links

- [Website](https://lassare.com)
- [Portal](https://portal.lassare.com)
- [GitHub](https://github.com/lassare-hq)

## License

MIT License
